...
const { deleteFromS3, generatePresignedUrl } = require("../config/s3");
...
// Updated function using AWS SDK v3
exports.getPersonalizedRecommendations = async (req, res) => {
  try {
    ...
    // Verify customer exists in your database
    ...
    console.log(`Getting recommendations for customer: ${cardCode}`);

    try {
      // Prepare Lambda function parameters
      const command = new InvokeCommand({
        FunctionName: "customer-recommendations2", // Your Lambda function name
      ...
      const responsePayload = new TextDecoder().decode(lambdaResult.Payload);
      const lambdaResponse = JSON.parse(responsePayload);

      if (lambdaResponse.statusCode === 200) {
        const recommendations = JSON.parse(lambdaResponse.body);
      ...
      res.status(500).json({
        success: false,
        message: "Error calling recommendation service",
...
// Alternative HTTP method (if you prefer to use axios instead of Lambda SDK)
exports.getPersonalizedRecommendationsHTTP = async (req, res) => {
  try {
    const { cardCode } = req.params;
    ...
    // Make HTTP request to Lambda via API Gateway (if you set up API Gateway)
    const LAMBDA_API_URL = process.env.LAMBDA_API_URL; // Your API Gateway URL

    if (!LAMBDA_API_URL) {
      return res.status(500).json({
        success: false,
        message: "Lambda API URL not configured",
      });
    }
    ...
    console.error("Error getting recommendations via HTTP:", error);
    ...
    if (error.response) {
      return res.status(error.response.status || 500).json({
        success: false,
        message: "Error from recommendation service",
        error: error.response.data?.error || "External service error",
      });
    }
...
// Get customer order history (unchanged - no AWS SDK needed)
exports.getCustomerOrderHistory = async (req, res) => {
  try {
    ...
    // Get customer's invoices with items
    const invoices = await Invoice.find({ CardCode: cardCode })
    ...
    res.status(500).json({
      success: false,
      message: "Error getting customer order history",
      error: error.message,
    });
  }
};
...
exports.comprehensiveCustomerUpdate = async (req, res) => {
  try {
    ...
    if (fileExt === ".xlsx" || fileExt === ".xls") {
      ...
      console.log(
        `Processed ${customersData.length} records in ${Date.now() - startTime
        }ms`
      );
    } else {
    ...
    console.log(
      `Found ${existingCustomers.length} existing customers in ${Date.now() - startTime
      }ms`
    );
    ...
    for (const [email, customers] of Object.entries(emailGroups)) {
      ...
      if (sapCustomers.length > 0 && nonSapCustomers.length > 0) {
        ...
        await Customer.findByIdAndUpdate(secondaryCustomer._id, {
          status: "inactive",
          notes: `Merged with ${primaryCustomer.CardCode} (${primaryCustomer.CardName
...
exports.removeZeroValueAbandonedCarts = async (req, res) => {
  try {
    ...
    sampleCarts.forEach((cart, index) => {
      console.log(
        `${index + 1}. CartID: ${cart.cartId}, Email: ${cart.customerEmail
        }, TotalInc: ${cart.totalIncludingTaxes}, TotalExc: ${cart.totalExcludingTaxes
        }, Products: ${cart.products?.length || 0}, Status: ${cart.status
...
exports.removeCustomersWithSCardCode = async (req, res) => {
  try {
    ...
    sampleCustomers.forEach((customer, index) => {
      console.log(
        `${index + 1}. CardCode: ${customer.CardCode}, Name: ${customer.CardName
        }, Email: ${customer.Email}, Status: ${customer.status}, Type: ${customer.customerType
        }`
      );
    });
...
exports.comprehensiveCustomerUpdateWithDelivery = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    console.log(
      "Processing comprehensive customer update file:",
      req.file.path
    );
    const startTime = Date.now();
    ...
    if (fileExt === ".xlsx" || fileExt === ".xls") {
      ...
      // Map the data to our customer format with delivery address
      customersData = data
        .map((row) => {
          const cardCode = (row["BP Code"] || "").toString().trim();
          if (!cardCode) return null;
          ...
          return {
            ...
            billingAddress: {
              ...
              zipCode: (row["Bill-to Postcode"] || "").toString().trim(),
              city: (row["Bill-to City"] || "").toString().trim(),
              country: (row["Bill-to Country/Region"] || "FR")
                .toString()
                .trim(),
            },
            ...
            deliveryAddress: {
              street: (row["Ship-to Street"] || "").toString().trim(),
              zipCode: (row["Ship-to Postcode"] || "").toString().trim(),
              city: (row["Ship-to City"] || "").toString().trim(),
              country: (row["Ship-to Country/Region"] || "FR")
                .toString()
                .trim(),
            },
    ...
    // Fast processing with delivery address handling
    for (const customerData of customersData) {
      const existing = existingMap.get(customerData.CardCode);

      if (!existing) {
        // NEW CUSTOMER - create with both addresses
        newCustomers.push({
          CardCode: customerData.CardCode,
          CardName: customerData.CardName,
          Email: customerData.Email,
        ...
        // EXISTING CUSTOMER - build comprehensive update
        const updateDoc = {
          CardName: customerData.CardName,
          outstandingBalance: customerData.outstandingBalance,
          prestashopAcc: customerData.prestashopAcc,
        ...
        // Always update billing address if data is provided
        if (
          customerData.billingAddress.street ||
          customerData.billingAddress.city
        ) {
          updateDoc.address = customerData.billingAddress;
        }

        // Always update delivery address if data is provided
        if (
          customerData.deliveryAddress.street ||
          customerData.deliveryAddress.city
        ) {
          updateDoc.deliveryAddress = customerData.deliveryAddress;
        }
    ...
    res.status(200).json({
      success: true,
      message: `Comprehensive update with delivery addresses completed in ${totalTime}ms: ${results.created} created, ${results.updated} updated`,
      results: {
        ...results,
        processingTimeMs: totalTime,
        recordsPerSecond: Math.round((customersData.length / totalTime) * 1000),
        summary: {
          newCustomers: results.created,
          updatedCustomers: results.updated,
          totalProcessed: customersData.length,
          withDeliveryAddresses: customersData.filter(
            (c) => c.deliveryAddress.street || c.deliveryAddress.city
          ).length,
          withBillingAddresses: customersData.filter(
            (c) => c.billingAddress.street || c.billingAddress.city
          ).length,
        },
      },
    });
  } catch (error) {
    console.error("Comprehensive customer update error:", error);
    ...
    res.status(500).json({
      success: false,
      message: "Error updating customers with delivery addresses",
      error: error.message,
    });
  }
};

exports.updateCustomerDeliveryAddresses = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: "Please upload a file containing customer delivery addresses",
      });
    }

    console.log("Processing delivery address update file:", req.file.path);
    const startTime = Date.now();
    ...
    if (fileExt === ".xlsx" || fileExt === ".xls") {
      ...
      // Map data to delivery address format
      deliveryData = data.map((row) => {
        return {
          CardCode: (row["BP Code"] || "").toString().trim(),
          CardName: (row["BP Name"] || "").toString().trim(),
          deliveryAddress: {
            street: (row["Ship-to Street"] || "").toString().trim(),
            zipCode: (row["Ship-to Postcode"] || "").toString().trim(),
            city: (row["Ship-to City"] || "").toString().trim(),
            country: (row["Ship-to Country/Region"] || "FR").toString().trim(),
          },
        };
      });
    } else if (fileExt === ".csv") {
      // Process CSV file
      deliveryData = await new Promise((resolve, reject) => {
        const results = [];
        fs.createReadStream(req.file.path)
          .pipe(csv())
          .on("data", (row) => {
    ...
    // Filter out entries without CardCode or delivery address
    ...
    console.log(
      `Prepared ${deliveryData.length} delivery address records for update`
    );

    if (deliveryData.length === 0) {
      return res.status(400).json({
        success: false,
        message: "No valid delivery address data found in the file",
      });
    }
    ...
    // Get all customer CardCodes for efficient lookup
    const cardCodes = deliveryData.map((item) => item.CardCode);
    const existingCustomers = await Customer.find({
      CardCode: { $in: cardCodes },
    })
      .select("CardCode")
    ...
    const existingCardCodes = new Set(existingCustomers.map((c) => c.CardCode));
    ...
    if (bulkOperations.length > 0) {
      console.log(
        `Executing ${bulkOperations.length} delivery address updates`
      );
      ...
      // Update results based on actual bulk operation results
      results.updated = bulkResult.modifiedCount;

      if (bulkResult.writeErrors && bulkResult.writeErrors.length > 0) {
        results.errors = bulkResult.writeErrors.map((err) => ({
          CardCode: err.op?.filter?.CardCode || "Unknown",
          error: err.errmsg,
        }));
      }
    ...
    const totalTime = Date.now() - startTime;
    console.log(`Delivery address update completed in ${totalTime}ms`);

    res.status(200).json({
      success: true,
      message: `Delivery address update completed in ${totalTime}ms`,
      results: {
        ...results,
        processingTimeMs: totalTime,
        summary: {
          ...
          updatedCustomers: results.updated,
          customersNotFound: results.notFound,
    ...
    res.status(500).json({
      success: false,
      message: "Error updating customer delivery addresses",
      error: error.message,
    });
  }
};

exports.startRestaurantAnalysis = async (req, res) => {
  try {
    const { customerId } = req.params;
    const { restaurant_url } = req.body;

    // Validate URL
    if (!restaurant_url || !restaurant_url.includes("ubereats.com")) {
      return res.status(400).json({
        success: false,
        error: "Valid UberEats URL is required",
      });
    }

    // Find customer by ID or CardCode
    let customer;
    if (customerId.length === 24) {
      // MongoDB ObjectId
      customer = await Customer.findById(customerId);
    } else {
      // Assume it's a CardCode
      customer = await Customer.findOne({ CardCode: customerId });
    }
    ...
    // Start analysis with restaurant matcher API
    const SCRAPER_API_URL =
      process.env.SCRAPER_API_URL || "https://hfs-ws.onrender.com";

    console.log(
      `Starting restaurant analysis for customer ${customer.CardCode} with URL: ${restaurant_url}`
    );

    const response = await axios.post(
      `${SCRAPER_API_URL}/match-restaurant-inventory`,
      {
    ...
    if (response.data.success) {
      // Save or update restaurant analysis record
      const analysisData = {
      ...
      const existingAnalysis = await RestaurantAnalysis.findOne({
        customerId: customer._id,
      });

      if (existingAnalysis) {
        // Update existing analysis
        await RestaurantAnalysis.findByIdAndUpdate(
          existingAnalysis._id,
          analysisData
        );
        console.log(
          `Updated existing analysis for customer ${customer.CardCode}`
        );
      } else {
        // Create new analysis
        await RestaurantAnalysis.create(analysisData);
        console.log(`Created new analysis for customer ${customer.CardCode}`);
      }

      // Update customer with restaurant URL
      await Customer.findByIdAndUpdate(customer._id, {
        uberEatsUrl: restaurant_url,
        lastRestaurantAnalysis: new Date(),
      });

      res.status(202).json({
        success: true,
        job_id: response.data.job_id,
        message: "Restaurant analysis started successfully",
        customer: {
          id: customer._id,
          CardCode: customer.CardCode,
          CardName: customer.CardName,
    ...
    console.error("Error starting restaurant analysis:", error);

    if (error.code === "ECONNABORTED") {
      return res.status(408).json({
        success: false,
        error: "Request timeout - analysis service may be slow",
      });
    }
...
exports.getRestaurantAnalysis = async (req, res) => {
  try {
    const { customerId } = req.params;
    ...
    const analysis = await RestaurantAnalysis.findOne({
      customerId: customer._id,
    }).sort({ created_at: -1 });
    ...
    if (!analysis) {
      return res.status(404).json({
        success: false,
        error: "No restaurant analysis found for this customer",
      });
    }

    // If analysis is still running, check status and potentially get results
    if (analysis.status === "running" || analysis.status === "processing") {
      try {
        const SCRAPER_API_URL =
          process.env.SCRAPER_API_URL || "https://hfs-ws.onrender.com";
        ...
        if (statusResponse.data.success) {
          ...
          // Update analysis with latest status
          analysis.status = jobStatus.status;
          analysis.progress = jobStatus.progress;
          analysis.runtime_seconds = jobStatus.runtime_seconds;
          analysis.updated_at = new Date();

          if (jobStatus.status === "completed" && jobStatus.result) {
            // Job completed, save results
            analysis.restaurant_name = jobStatus.result.restaurant_name;
            analysis.total_matches = jobStatus.result.total_matches;
            analysis.restaurant_categories =
              jobStatus.result.restaurant_categories;
            analysis.llm_categorization_used =
              jobStatus.result.llm_categorization_used;
            analysis.openai_enhancement_used =
              jobStatus.result.openai_enhancement_used;
            analysis.matching_results = jobStatus.result.matching_results;
            analysis.completed_at = new Date();

            console.log(
              `Analysis completed for customer ${customer.CardCode}: ${analysis.total_matches} matches found`
            );
          } else if (jobStatus.status === "failed") {
            analysis.error_message =
              jobStatus.progress?.error || "Analysis failed";
            console.log(
              `Analysis failed for customer ${customer.CardCode}: ${analysis.error_message}`
            );
          }
        ...
        console.error("Error checking analysis status:", statusError);
        // Continue with existing data - don't fail the request
      }
    }
...
// Delete customer restaurant analysis
exports.deleteRestaurantAnalysis = async (req, res) => {
  try {
    const { customerId } = req.params;
    ...
    const deletedAnalysis = await RestaurantAnalysis.findOneAndDelete({
      customerId: customer._id,
    });

    // Remove restaurant URL from customer
    await Customer.findByIdAndUpdate(customer._id, {
      $unset: {
        uberEatsUrl: 1,
        lastRestaurantAnalysis: 1,
      },
    });

    res.json({
      success: true,
      message: "Restaurant analysis deleted successfully",
      deleted: deletedAnalysis ? true : false,
    });
  } catch (error) {
    console.error("Error deleting restaurant analysis:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
      details: error.message,
...
exports.saveRestaurantUrl = async (req, res) => {
  try {
    const { customerId } = req.params;
    const { uberEatsUrl } = req.body;

    // Validate URL
    if (!uberEatsUrl || !uberEatsUrl.includes("ubereats.com")) {
      return res.status(400).json({
        success: false,
        error: "Valid UberEats URL is required",
      });
    ...
    res.json({
      success: true,
      message: "Restaurant URL saved successfully",
      data: {
        id: customer._id,
        CardCode: customer.CardCode,
        CardName: customer.CardName,
        uberEatsUrl: customer.uberEatsUrl,
      },
    });
  } catch (error) {
    console.error("Error saving restaurant URL:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
      details: error.message,
...
exports.getAnalysisStatus = async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${SCRAPER_API_URL}/inventory-matching-job/${jobId}`,
      {
        timeout: 10000,
      }
    );

    // Update local analysis record if found
    const analysis = await RestaurantAnalysis.findOne({ job_id: jobId });
    if (analysis) {
      analysis.status = response.data.status;
      analysis.updated_at = new Date();

      if (response.data.status === "completed") {
        analysis.results = response.data.results;
      } else if (response.data.status === "failed") {
        analysis.error = response.data.error;
      }
...
// Get all restaurant analyses (admin function)
exports.getAllRestaurantAnalyses = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    ...
    const total = await RestaurantAnalysis.countDocuments(query);
    const analyses = await RestaurantAnalysis.find(query)
      .sort({ created_at: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error fetching restaurant analyses:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
      details: error.message,
...
exports.uploadCustomerDocument = async (req, res) => {
  try {
    ...
    console.log(
      `Uploading document for customer ${customerId} with type ${documentType}`
    );
    ...
    const validDocumentTypes = ["companyDoc", "managerId", "otherDocuments"];
    if (!validDocumentTypes.includes(documentType)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid document type. Must be: companyDoc, managerId, or otherDocuments",
      });
    }
    ...
    if (documentType === "companyDoc") {
      // If there's an existing company document, delete it from S3
      if (customer.documents?.companyDoc?.s3Key) {
        try {
          await deleteFromS3(customer.documents.companyDoc.s3Key);
        } catch (error) {
      ...
      customer.documents.companyDoc = documentData;
    } else if (documentType === "managerId") {
      // If there's an existing manager ID document, delete it from S3
      if (customer.documents?.managerId?.s3Key) {
        try {
          await deleteFromS3(customer.documents.managerId.s3Key);
        } catch (error) {
          console.error("Error deleting old manager ID document:", error);
        }
      }
      ...
      customer.documents.managerId = documentData;
    } else if (documentType === "otherDocuments") {
      // Initialize documents object and otherDocuments array if they don't exist
      if (!customer.documents) {
        customer.documents = { otherDocuments: [] };
      }
      if (!customer.documents.otherDocuments) {
        customer.documents.otherDocuments = [];
      }

      customer.documents.otherDocuments.push(documentData);
    }

    // Save customer with new document
    await customer.save();
...
// Get customer documents with presigned URLs
exports.getCustomerDocuments = async (req, res) => {
  try {
    ...
    if (customer.documents) {
      // Generate presigned URLs for company document
      if (customer.documents.companyDoc) {
        console.log(
          `Generating presigned URL for company document: ${customer.documents.companyDoc.s3Key}`
        );
        try {
          const presignedUrl = await generatePresignedUrl(
            customer.documents.companyDoc.s3Key
          );
          documents.companyDoc = {
            ...customer.documents.companyDoc.toObject(),
            presignedUrl,
          };
        } catch (error) {
          console.error(
            "Error generating presigned URL for company doc:",
            error
          );
          documents.companyDoc = customer.documents.companyDoc.toObject();
        }
      }

      // Generate presigned URLs for manager ID document
      if (customer.documents.managerId) {
        try {
          const presignedUrl = await generatePresignedUrl(
            customer.documents.managerId.s3Key
          );
          documents.managerId = {
            ...customer.documents.managerId.toObject(),
            presignedUrl,
          };
        } catch (error) {
          console.error(
            "Error generating presigned URL for manager ID:",
            error
          );
          documents.managerId = customer.documents.managerId.toObject();
        }
      }
      ...
      if (
        customer.documents.otherDocuments &&
        customer.documents.otherDocuments.length > 0
      ) {
        for (const doc of customer.documents.otherDocuments) {
          try {
            const presignedUrl = await generatePresignedUrl(doc.s3Key);
            documents.otherDocuments.push({
              ...doc.toObject(),
              presignedUrl,
            });
          } catch (error) {
            console.error(
              "Error generating presigned URL for other document:",
              error
            );
            documents.otherDocuments.push(doc.toObject());
          }
        }
      }
    }
...
exports.deleteCustomerDocument = async (req, res) => {
  try {
    ...
    const { documentId } = req.query; // For other documents array
    ...
    if (documentType === "companyDoc") {
      if (customer.documents?.companyDoc) {
        documentToDelete = customer.documents.companyDoc;
        updateOperation = { $unset: { "documents.companyDoc": 1 } };
      }
    } else if (documentType === "managerId") {
      if (customer.documents?.managerId) {
        documentToDelete = customer.documents.managerId;
        updateOperation = { $unset: { "documents.managerId": 1 } };
      }
    } else if (documentType === "otherDocuments" && documentId) {
      if (customer.documents?.otherDocuments) {
        const docIndex = customer.documents.otherDocuments.findIndex(
          (doc) => doc._id.toString() === documentId
        );
        if (docIndex !== -1) {
          documentToDelete = customer.documents.otherDocuments[docIndex];
          updateOperation = {
            $pull: {
              "documents.otherDocuments": { _id: documentId },
            },
          };
        }
      }
    ...
    // Delete from S3
    try {
      await deleteFromS3(documentToDelete.s3Key);
    } catch (s3Error) {
...
exports.getDocumentDownloadUrl = async (req, res) => {
  try {
    ...
    if (documentType === "companyDoc") {
      document = customer.documents?.companyDoc;
    } else if (documentType === "managerId") {
      document = customer.documents?.managerId;
    } else if (documentType === "otherDocuments" && documentId) {
      if (customer.documents?.otherDocuments) {
        document = customer.documents.otherDocuments.find(
          (doc) => doc._id.toString() === documentId
        );
      }
    }
